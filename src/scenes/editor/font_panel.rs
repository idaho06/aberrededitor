...
pub(super) fn draw_font_editor(
    ...
    if !signals.flags.contains(sig::UI_FONT_STORE_OPEN) {
        return (false, false);
    }
    ...
    ui.window("Font Store")
        ...
        .build(|| {
            ui.child_window("##font_list").size([0.0, -80.0]).build(|| {
                ...
                for key in &sorted_keys {
                    ...
                    if ui.is_item_clicked() {
                        open_texture_viewer(intents, sig::TEXTURE_VIEWER_SOURCE_FONT, key.as_str());
                    }
                    ui.same_line();
                    ...
                    ui.group(|| {
                        ...
                        if ui.small_button("Rename") {
                            intents.set_string(sig::FONT_RENAME_SRC, key.as_str());
                            intents.set_string(sig::FONT_RENAME_BUF, key.as_str());
                            open_rename_popup = true;
                        }
                        ui.same_line();
                        if ui.small_button("Remove") {
                            intents.set_string(sig::FONT_REMOVE_KEY, key.as_str());
            ...
            if ui
                ...
                .build()
            {
                intents.set_scalar(sig::FONT_ADD_SIZE_BUF, add_size);
            }
        });
    ...
    ui.modal_popup_config("Rename Key##font_store")
        ...
        .build(|| {
            ...
            if ui
                ...
                .build()
            {
                intents.set_string(sig::FONT_RENAME_BUF, buf.as_str());
            }

            ui.spacing();
            ui.separator();
            if ui.button("OK##font_rename_ok") {
                intents.set_flag(sig::ACTION_FONT_RENAME);
    ...
    ui.modal_popup_config("Remove Font##font_store")
        ...
        .build(|| {
            ...
            if ui.button("Yes##font_remove_yes") {
                intents.set_flag(sig::ACTION_FONT_REMOVE);
                ui.close_current_popup();
            }
            ui.same_line();
