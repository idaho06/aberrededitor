...
pub(super) fn draw_texture_editor(
    ...
    if !signals.flags.contains(sig::UI_TEXTURE_EDITOR_OPEN) {
        return (false, false);
    }
    ...
    ui.window("Texture Store")
        ...
        .build(|| {
                    ...
                    for key in &sorted_keys {
                        ...
                        ui.group(|| {
                            ...
                            if ui.small_button("Rename") {
                                intents.set_string(sig::TEX_RENAME_SRC, key.as_str());
                                intents.set_string(sig::TEX_RENAME_BUF, key.as_str());
                                open_rename_popup = true;
                            }
                            ui.same_line();
                            if ui.small_button("Remove") {
                                intents.set_string(sig::TEX_REMOVE_KEY, key.as_str());
                            ...
                            if ui.combo_simple_string(
                                ...
                                intents.set_flag(sig::ACTION_TEXTURE_CHANGE_FILTER);
                            }
                        });
                    }
                });
            ...
            let mut add_key = signals
                .strings
                .get(sig::TEX_ADD_KEY_BUF)
                .cloned()
                .unwrap_or_default();
            ui.set_next_item_width(ui.content_region_avail()[0] - 85.0);
            if ui.input_text("##add_key", &mut add_key).build() {
                intents.set_string(sig::TEX_ADD_KEY_BUF, add_key.as_str());
            }
            ui.same_line();
            if ui.button("Browse...##add") {
                intents.set_flag(sig::ACTION_TEXTURE_ADD_BROWSE);
            }
        });
...
pub(super) fn draw_texture_modals(
    ui: &imgui::Ui,
    signals: &SignalSnapshot,
    intents: &mut SignalIntents,
) {
    ui.modal_popup_config("Rename Key##texture_editor")
        ...
        .build(|| {
            ...
            let mut buf = signals
                .strings
                .get(sig::TEX_RENAME_BUF)
                .cloned()
                .unwrap_or_default();
            if ui.input_text("New key##rename_input", &mut buf).build() {
                intents.set_string(sig::TEX_RENAME_BUF, buf.as_str());
            }

            ui.spacing();
            ui.separator();
            if ui.button("OK##rename_ok") {
                intents.set_flag(sig::ACTION_TEXTURE_RENAME);
    ...
    ui.modal_popup_config("Remove Texture##texture_editor")
        ...
        .build(|| {
            ...
            if ui.button("Yes##remove_yes") {
                intents.set_flag(sig::ACTION_TEXTURE_REMOVE);
                ui.close_current_popup();
            }
            ui.same_line();
